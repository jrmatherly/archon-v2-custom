    ...
    async def manage_document(
        ...
        try:
            ...
            if action == "add":
                ...
                if not title:
                    return json.dumps(
                        {"success": False, "error": "title is required for add action"}
                    )
                ...
                if not doc_id:
                    return json.dumps(
                        {"success": False, "error": "doc_id is required for get action"}
                    )
                ...
                if content is not None:
                    ...
                    async with httpx.AsyncClient(timeout=timeout) as client:
                        get_response = await client.get(
                            urljoin(
                                api_url, f"/api/projects/{project_id}/docs/{doc_id}"
                            )
                        )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    return json.dumps(
                        {"success": False, "error": "Failed to get project features"}
                    )
