...
class BackgroundTaskManager:
    ...
    def _run_with_progress(
        self, task_func: Callable, task_args: tuple, progress_queue: Queue, task_id: str
    ) -> Any:
        ...
        try:
            ...
            progress_queue.put({"status": "error", "percentage": -1, "error": str(e)})
            raise

    async def _emit_progress(self, task_id: str, queue: Queue, callback: Callable):
        ...
        while True:
            try:
                if not queue.empty():
                    ...
                    logger.info(
                        f"Progress update from queue for task {task_id}: status={update.get('status')}, percentage={update.get('percentage')}"
                    )
                    ...
                    if "percentage" in update:
                        ...
                        self.task_metadata[task_id]["status"] = update.get(
                            "status", "running"
                        )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Progress callback error for task {task_id}: {callback_error}"
                        )
                        logger.error(
                            "Continuing task execution despite callback failure"
                        )

                    # Exit on completion
                    if update.get("status") in ["complete", "error"]:
                        logger.debug(
                            f"Task {task_id} reached terminal state: {update.get('status')}"
                        )
                        break
                    ...
                    if time_since_heartbeat >= heartbeat_interval:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Heartbeat callback error for task {task_id}: {e}"
                            )
