...
async def get_llm_client(
    provider: Optional[str] = None, use_embedding_provider: bool = False
):
    """
    ...
    try:
        ...
        if credential_service._cache_initialized:
            ...
            if provider == "openai":
                ...
                if isinstance(openai_key_data, dict) and openai_key_data.get(
                    "is_encrypted"
                ):
                    # Decrypt the API key
                    try:
                        api_key = credential_service._decrypt_value(
                            openai_key_data["encrypted_value"]
                        )
                    except Exception as e:
                ...
                if isinstance(google_key_data, dict) and google_key_data.get(
                    "is_encrypted"
                ):
                    # Decrypt the API key
                    try:
                        api_key = credential_service._decrypt_value(
                            google_key_data["encrypted_value"]
                        )
                    except Exception as e:
            ...
            logger.debug(
                f"Sync provider config - Provider: {provider}, API key present: {bool(api_key)}"
            )
            ...
            logger.warning(
                "Credential service cache not initialized, using environment variables"
            )
