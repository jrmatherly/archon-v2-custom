...
class SearchService:
    ...
    async def perform_rag_query(
        ...
        with safe_span(
            ...
            try:
                ...
                with safe_span("format_response"):
                    ...
                    for i, result in enumerate(results):
                        try:
                            ...
                            logger.warning(
                                f"Failed to format result {i}: {format_error}"
                            )
                            continue
                ...
                logger.info(
                    f"RAG query completed successfully - {len(formatted_results)} results found"
                )
                ...
                logger.error(
                    f"RAG query failed: {e} (type: {type(e).__name__}) for query: {query[:50]}"
                )
    ...
    async def search_code_examples_service(
        ...
        try:
            ...
            extract_code_examples_enabled = self.get_bool_setting(
            ...
            if use_hybrid_search:
                ...
                keyword_query = (
                    self.supabase_client.from_("code_examples")
                    .select(
