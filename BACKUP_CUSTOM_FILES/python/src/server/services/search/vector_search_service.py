...
def search_documents(
    ...
    with safe_span(
        ...
        try:
            logger.info(
                f"Document search started - query: {query[:100]}{'...' if len(query) > 100 else ''}, match_count: {match_count}, filter: {filter_metadata}"
            )
            ...
            with safe_span("prepare_rpc_params"):
                ...
                if filter_metadata:
                    ...
                    span.set_attribute(
                        "filter_keys",
                        list(filter_metadata.keys()) if filter_metadata else [],
                    )
                else:
            ...
            with safe_span("supabase_rpc_call"):
                logger.debug(
                    f"Calling Supabase RPC function: match_crawled_pages, params: {list(rpc_params.keys())}"
                )
                ...
                span.set_attribute(
                    "raw_results_count", len(response.data) if response.data else 0
                )
            ...
            if results_count == 0:
                logger.warning(
                    f"Document search returned 0 results - query: {query[:100]}{'...' if len(query) > 100 else ''}, raw_count: {len(response.data) if response.data else 0}, filter: {filter_metadata}"
                )
            else:
                logger.info(
                    f"Document search completed - query: {query[:100]}{'...' if len(query) > 100 else ''}, results: {results_count}, raw_count: {len(response.data) if response.data else 0}"
                )
            ...
            logger.error(
                f"Document search failed - query: {query[:100]}{'...' if len(query) > 100 else ''}, error: {e} ({type(e).__name__})"
            )
...
async def search_documents_async(
    ...
    with safe_span(
        ...
        try:
            logger.info(
                f"Document search started (async) - query: {query[:100]}{'...' if len(query) > 100 else ''}, match_count: {match_count}, filter: {filter_metadata}"
            )
            ...
            logger.info(
                f"Document search completed (async) - query: {query[:100]}{'...' if len(query) > 100 else ''}, results: {results_count}"
            )
            ...
            logger.error(
                f"Document search failed (async) - query: {query[:100]}{'...' if len(query) > 100 else ''}, error: {e} ({type(e).__name__})"
            )
