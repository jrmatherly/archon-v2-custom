...
def run_async_in_thread(coro: Coroutine, timeout: float = 30.0) -> Any:
    ...
    if _main_event_loop and not _main_event_loop.is_closed():
        logger.info(
            f"run_async_in_thread: Using main event loop via run_coroutine_threadsafe (loop={_main_event_loop})"
        )
        try:
            future = asyncio.run_coroutine_threadsafe(coro, _main_event_loop)
            result = future.result(timeout=timeout)
            logger.info(
                "run_async_in_thread: Successfully completed via main event loop"
            )
            return result
        ...
        logger.warning(
            f"run_async_in_thread: Main event loop not available - loop={_main_event_loop}, closed={_main_event_loop.is_closed() if _main_event_loop else 'N/A'}"
        )

    # No fallback - if main event loop is not available, fail fast
    logger.error(
        "run_async_in_thread: Main event loop not available - cannot run async code in thread!"
    )
    raise RuntimeError(
...
class BlockingEmbeddingWrapper:
    ...
    def generate_embeddings_batch(
        texts: List[str], progress_queue: Queue
    ) -> List[List[float]]:
        """
