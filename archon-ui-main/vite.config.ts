...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
	...
	const mcpPort = process.env.ARCHON_MCP_PORT || "8051";
	...
	return {
					...
					server.middlewares.use("/api/run-tests", (req: any, res: any) => {
						...
						testProcess.stdout?.on("data", (data) => {
							...
							lines.forEach((line: string) => {
								...
								res.write(
									`data: ${JSON.stringify({ type: "output", message: line, timestamp: new Date().toISOString() })}\n\n`,
								);
							});
						...
						testProcess.stderr?.on("data", (data) => {
							...
							lines.forEach((line: string) => {
								...
								res.write(
									`data: ${JSON.stringify({ type: "output", message: cleanLine, timestamp: new Date().toISOString() })}\n\n`,
								);
							});
						});
						...
						testProcess.on("close", (code) => {
							res.write(
								`data: ${JSON.stringify({
									...
									message:
										code === 0
											? "Tests completed and results generated!"
											: "Tests failed",
							...
							testProcess.on("close", (code) => {
								res.write(
									`data: ${JSON.stringify({
										...
										message:
											code === 0
												? "Tests completed with coverage and results generated!"
												: "Tests failed",
										timestamp: new Date().toISOString(),
									})}\n\n`,
								);
		...
		server: {
			...
			allowedHosts: [
				...
				"127.0.0.1",
				"0.0.0.0",
				"aanai-archon.aarons.com",
				".aarons.com", // Allow all subdomains of aarons.com
				//	"all", // Allow all hosts (use with caution in production)
			],
			proxy: {
				"/api": {
		...
		define: {
			...
			// MCP service URL configuration
			"import.meta.env.VITE_MCP_URL": JSON.stringify(
				env.VITE_MCP_URL || `http://${host}:${mcpPort}/mcp`,
			),
		},
		resolve: {
			alias: {
