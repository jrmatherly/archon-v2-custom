	...
	const loadConfiguration = async () => {
		try {
			...
			setConfig({
				...
				host: "localhost",
				port:
					typeof defaultPort === "string" ? parseInt(defaultPort) : defaultPort,
			});
		}
	};
									...
									<div className="flex gap-2 items-center">
										{serverStatus.status === "stopped" ? (
											...
											<Button
												...
												disabled={
													isStopping || serverStatus.status !== "running"
												}
												variant="primary"
												accentColor="pink"
									...
									<div className="border-t border-gray-200 dark:border-zinc-800 pt-4">
										...
										<div className="mb-4">
											...
											<ul className="text-sm text-gray-600 dark:text-zinc-400 space-y-1">
												{getIDEInstructions(selectedIDE).steps.map(
													(step, index) => (
											...
											serverStatus.status === "running" && (
												<div className="mt-4">
													...
													<p className="text-xs text-gray-500 dark:text-zinc-500 mt-2 text-center">
														Requires Cursor to be installed and will open a
														deeplink
													</p>
												</div>
											)}
