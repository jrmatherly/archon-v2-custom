...
export function useTaskSocket(options: UseTaskSocketOptions) {
	...
	const componentIdRef = useRef<string>(
		`task-socket-${Math.random().toString(36).substring(7)}`,
	);
	...
	useEffect(() => {
		...
		const initializeConnection = async () => {
			try {
				console.log(
					`[USE_TASK_SOCKET] Initializing connection for project: ${projectId}`,
				);

				// Register handlers first
				taskSocketService.registerHandlers(
				...
				console.log(
					`[USE_TASK_SOCKET] Successfully initialized for project: ${projectId}`,
				);
			} catch (error) {
				console.error(
					`[USE_TASK_SOCKET] Failed to initialize for project ${projectId}:`,
					error,
				);
			}
		};
	...
	useEffect(() => {
		if (isInitializedRef.current && currentProjectIdRef.current === projectId) {
			console.log(
				`[USE_TASK_SOCKET] Updating handlers for component: ${componentIdRef.current}`,
			);
			taskSocketService.registerHandlers(
				componentIdRef.current,
				memoizedHandlers(),
	...
	useEffect(() => {
		...
		if (isInitializedRef.current && currentProjectIdRef.current !== projectId) {
			console.log(
				`[USE_TASK_SOCKET] Project changed from ${currentProjectIdRef.current} to ${projectId}`,
			);
			...
			const switchProject = async () => {
				try {
					...
					currentProjectIdRef.current = projectId;
					console.log(
						`[USE_TASK_SOCKET] Successfully switched to project: ${projectId}`,
					);
				} catch (error) {
					console.error(
						`[USE_TASK_SOCKET] Failed to switch to project ${projectId}:`,
						error,
					);
				}
			};
	...
	return {
		...
		getCurrentProjectId:
			taskSocketService.getCurrentProjectId.bind(taskSocketService),
	};
}
