...
class MCPClientService {
	...
	async discoverClientTools(clientId: string): Promise<ToolsResponse> {
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}/tools/discover`,
			{
				method: "POST",
			},
		);
	 ...
	 */
	async connectMultipleClients(
		clientIds: string[],
	): Promise<Array<{ clientId: string; success: boolean; message: string }>> {
		...
		return results.map((result, index) =>
			result.status === "fulfilled"
				...
				: {
						...
						success: false,
						message: result.reason?.message || "Failed to connect",
					},
		);
	}
	...
	async getAllClientStatuses(): Promise<
		Array<{ client: MCPClient; status: ClientStatus }>
	> {
	...
	async autoConnectClients(): Promise<
		Array<{ clientId: string; success: boolean; message: string }>
	> {
		const clients = await this.getClients();
	...
	async createArchonClient(): Promise<MCPClient> {
		const archonConfig: MCPClientConfig = {
			...
			transport_type: "http",
			connection_config: {
				url: import.meta.env.VITE_MCP_URL || "http://localhost:8051/mcp",
			},
			auto_connect: true,
	...
	async getOrCreateArchonClient(): Promise<MCPClient> {
		const clients = await this.getClients();
		const archonClient = clients.find(
			(client) => client.is_default || client.name === "Archon",
		);
