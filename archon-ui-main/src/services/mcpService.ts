...
class MCPService {
	...
	async updateClient(
		clientId: string,
		updates: Partial<MCPClientConfig>,
	): Promise<MCPClient> {
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	async connectClient(
		...
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}/connect`,
			{
				method: "POST",
			},
		);
	...
	async disconnectClient(
		...
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}/disconnect`,
			{
				method: "POST",
			},
		);
	...
	async getClientStatus(clientId: string): Promise<{
		...
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}/status`,
		);
	...
	async getClientTools(clientId: string): Promise<{
		...
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/${clientId}/tools`,
		);
	...
	async testClientConfig(
		...
		const response = await fetch(
			`${this.baseUrl}/api/mcp/clients/test-config`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(config),
	...
	async getAvailableTools(): Promise<MCPTool[]> {
		try {
			...
			console.log(
				"Successfully retrieved tools via MCP protocol:",
				validatedResult.tools.length,
			);
			return validatedResult.tools;
		} catch (mcpError) {
			console.warn(
				"Direct MCP call failed, falling back to backend endpoint:",
				mcpError,
			);
			...
			try {
				...
				if (response.ok) {
					...
					if (
						data.tools.length === 1 &&
						data.tools[0].name === "debug_placeholder"
					) {
						console.warn(
							"Backend returned debug placeholder - MCP tool introspection is not working",
						);
				...
				console.error("Both MCP protocol and backend endpoint failed:", {
					mcpError,
					backendError,
				});
				throw new Error(
					`Failed to retrieve tools: MCP protocol failed (${mcpError instanceof Error ? mcpError.message : mcpError}), backend also failed (${backendError instanceof Error ? backendError.message : backendError})`,
				);
			}
		}
	}
	...
	async getAllAvailableTools(): Promise<{
		...
		try {
			...
			const clientTools = await Promise.all(
				clients
					.filter(
						(client) => client.status === "connected" && !client.is_default,
					)
					.map(async (client) => {
						try {
			...
			const totalCount =
				archonTools.length +
				clientTools.reduce((sum, ct) => sum + ct.tools.length, 0);
...
export const getMCPTools = async () => {
	console.warn(
		"getMCPTools is deprecated. Use mcpService.getAvailableTools() instead.",
	);
