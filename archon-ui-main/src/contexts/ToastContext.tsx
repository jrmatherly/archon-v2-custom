	...
	const getGlassmorphismStyles = (type: Toast["type"]) => {
		switch (type) {
			...
			case "error":
				return {
					container:
						"backdrop-blur-xl bg-gradient-to-r from-red-50/95 to-pink-50/95 dark:from-red-950/90 dark:to-pink-950/90 border border-red-300/60 dark:border-red-500/40 shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_25px_-5px_rgba(0,0,0,0.6),0_10px_10px_-5px_rgba(0,0,0,0.3)]",
			...
			case "warning":
				return {
					...
					buttonColor:
						"text-yellow-600 hover:text-yellow-800 dark:text-yellow-300 dark:hover:text-yellow-100",
				};
			case "info":
			default:
				return {
				...
				<AnimatePresence>
					{toasts.map((toast) => (
						...
						>
							{getIcon(toast.type)}
							<p
								className={`flex-1 text-sm font-medium ${getGlassmorphismStyles(toast.type).textColor}`}
							>
								{toast.message}
							</p>
							<button
