...
class PRPViewerTester:
    ...
    async def capture_view_content(self, page: Page, view_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if view_type == "markdown":
                ...
                # Take screenshot
                screenshot_path = (
                    self.output_dir / f"{view_type}_{datetime.now().timestamp()}.png"
                )
    ...
    async def compare_views(
        ...
        # Compare sections
        markdown_sections = {
            s["text"].lower() for s in markdown_content.get("sections", [])
        }
        beautiful_sections = {
            s["text"].lower() for s in beautiful_content.get("sections", [])
        }
    ...
    async def test_document(
        self, browser: Browser, doc: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                await page.wait_for_selector(
                    'h2:has-text("Project Docs"), .prp-viewer, .milkdown-editor',
                    timeout=15000,
                )
            ...
            try:
                ...
                await page.wait_for_selector(
                    ".flex.gap-4 .cursor-pointer", timeout=5000
                )
            ...
            result["views"] = {
                "markdown": {
                    ...
                    "sections_found": len(
                        markdown_view.get("content", {}).get("sections", [])
                    )
                ...
                "beautiful": {
                    ...
                    "sections_found": len(
                        beautiful_view.get("content", {}).get("sections", [])
                    )
    ...
    def analyze_common_issues(self):
        ...
        self.results["summary"]["common_issues"] = [
            issue[0] for issue in common_issues[:5]
        ]
    ...
    def save_results(self):
        ...
        with open(summary_file, "w") as f:
            ...
            for issue_type, count in (
                self.results["summary"].get("issue_breakdown", {}).items()
            ):
            ...
            for doc in self.results["documents"]:
                if doc["issues"]:
                    ...
                    for issue in doc["issues"]:
                        f.write(
                            f"  - {issue['type']}: {issue.get('description', issue)}\n"
                        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test PRP Viewer rendering consistency"
    )
    parser.add_argument(
        "--project-id", required=True, help="UUID of the project to test"
    )
    parser.add_argument(
    ...
    print(
        "Note: Skipping connectivity check as Vite dev server may block direct HTTP requests"
    )
