...
class MCPServerManager:
    ...
    async def start_server(self) -> Dict[str, Any]:
        ...
        with safe_span("mcp_server_start") as span:
            ...
            try:
                ...
                mcp_logger.error(
                    f"Docker API error during MCP startup - error={str(e)}"
                )
                safe_set_attribute(span, "success", False)
    ...
    async def add_websocket(self, websocket: WebSocket):
        ...
        await websocket.send_json(
            {"type": "connection", "message": "WebSocket connected for log streaming"}
        )
...
async def stop_server():
    ...
    with safe_span("api_mcp_stop") as span:
        ...
        try:
            ...
            api_logger.info(
                f"MCP server stop API called - success={result.get('success', False)}"
            )
            safe_set_attribute(span, "success", result.get("success", False))
            return result
        except Exception as e:
...
async def get_status():
    ...
    with safe_span("api_mcp_status") as span:
        ...
        try:
            ...
            api_logger.debug(
                f"MCP server status checked - status={status.get('status')}"
            )
