...
async def broadcast_crawl_progress(progress_id: str, data: dict):
    ...
    try:
        ...
        print(
            f"📢 [SOCKETIO DEBUG] Room {progress_id} has {len(room_sids)} subscribers: {room_sids}"
        )
        print(
            f"📢 [SOCKETIO DEBUG] All rooms in namespace '/': {list(all_rooms.get('/', {}).keys())}"
        )

        # Log if the room doesn't exist
        if not room_sids:
            print(
                f"⚠️  [SOCKETIO DEBUG] WARNING: Room '{progress_id}' has no subscribers!"
            )
            logger.warning(
...
async def crawl_subscribe(sid, data=None):
    ...
    try:
        ...
        print(
            f"📥 [SOCKETIO DEBUG] Room '{progress_id}' membership confirmed: {progress_id in client_rooms}"
        )
        ...
        if hasattr(sio.manager, "rooms"):
            ...
            print(
                f"📥 [SOCKETIO DEBUG] Room '{progress_id}' now has {len(room_members)} members: {room_members}"
            )
...
async def crawl_unsubscribe(sid, data):
    ...
    if progress_id:
        ...
        print(
            f"🛑 [SOCKETIO DEBUG] Client {sid} requesting to unsubscribe from crawl progress {progress_id}"
        )
...
async def document_change(sid, data):
    ...
    try:
        ...
        await sio.emit(
            "error", {"message": f"Failed to process change: {str(e)}"}, to=sid
        )
...
async def document_batch_update(sid, data):
    ...
    try:
        ...
        await sio.emit(
            "error", {"message": f"Failed to process batch: {str(e)}"}, to=sid
        )
