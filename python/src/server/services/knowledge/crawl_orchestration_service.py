...
def register_orchestration(
    progress_id: str, orchestration: "CrawlOrchestrationService"
):
...
class CrawlOrchestrationService:
    ...
    def __init__(self, crawler, supabase_client, progress_id=None):
        ...
        self.crawling_service = CrawlingService(
            crawler=crawler, supabase_client=supabase_client
        )
    ...
    async def _async_orchestrate_crawl(self, request: Dict[str, Any], task_id: str):
        ...
        try:
            ...
            safe_logfire_info(
                f"Using source_id '{original_source_id}' from original URL '{url}'"
            )
            ...
            await self._handle_progress_update(
                task_id,
                {"status": "error", "percentage": -1, "log": f"Crawl failed: {str(e)}"},
            )
            # Unregister on error
            if self.progress_id:
                unregister_orchestration(self.progress_id)
    ...
    def _blocking_orchestrate_crawl(
        self, progress_queue: Queue, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            crawl_results, crawl_type = self._crawl_by_url_type_blocking(
                url, request, progress_queue
            )
            ...
            if request.get("extract_code_examples", True):
                ...
                try:
                    ...
                    safe_logfire_info(
                        f"Code extraction completed: {code_examples_count} examples stored"
                    )
                except Exception as e:
    ...
    async def _process_and_store_documents(
        ...
        for doc_index, doc in enumerate(crawl_results):
            ...
            safe_logfire_info(
                f"Using original source_id '{source_id}' for URL '{source_url}'"
            )
        ...
        if unique_source_ids:
            for source_id in unique_source_ids:
                try:
                    ...
                    safe_logfire_error(
                        f"Source verification failed for '{source_id}': {str(e)}"
                    )
                    raise
    ...
    def _extract_code_examples_blocking(
        self,
    ...
    def _crawl_by_url_type_blocking(
        self, url: str, request: Dict[str, Any], progress_queue: Queue
    ) -> tuple:
        """
                ...
                with urllib.request.urlopen(request, timeout=30) as response:
                    ...
                    if content_encoding == "gzip":
                        ...
                        try:
                            ...
                            raise ValueError(
                                "Cannot decompress brotli content - brotli module not installed"
                            )
                    ...
                    safe_logfire_info(
                        f"Text file downloaded, size: {len(content)} bytes"
                    )
                    progress_queue.put(
                        {
                            "status": "crawling",
                    ...
                    crawl_type = "text_file"
                    safe_logfire_info(
                        f"Successfully fetched text file: {url}, size: {len(content)} chars"
                    )
            except urllib.error.HTTPError as e:
                safe_logfire_error(
                    f"HTTP error fetching text file {url}: {e.code} {e.reason}"
                )
                if e.code == 403:
                    # Try with the crawler as fallback for 403 errors
                    safe_logfire_info(
                        "Got 403 error, falling back to crawler for text file"
                    )
                    progress_queue.put(
                        {
                            "status": "crawling",
                    ...
                    if result and result.get("success"):
                        ...
                        crawl_type = "text_file"
                        safe_logfire_info(
                            f"Successfully fetched text file with crawler: {url}"
                        )
                    else:
                        raise ValueError(
                            f"Failed to fetch text file even with crawler: HTTP {e.code} {e.reason}"
                        )
                else:
                    raise ValueError(
                        f"Failed to fetch text file: HTTP {e.code} {e.reason}"
                    )
            except Exception as e:
    ...
    def _process_and_store_documents_blocking(
        self,
        crawl_results: List[Dict],
        request: Dict[str, Any],
        crawl_type: str,
        ...
        safe_logfire_info(
            f"Document storage | documents={len(crawl_results)} | chunks={len(all_contents)}"
        )
        ...
        try:
            ...
            safe_logfire_info(
                "Document storage completed successfully with contextual embeddings"
            )
    ...
    def _extract_and_store_code_examples_blocking(
        self,
        crawl_results: List[Dict],
        ...
        """
        safe_logfire_error(
            "_extract_and_store_code_examples_blocking called - this method was removed!"
        )
        return 0
