...
def _get_model_choice() -> str:
    ...
    try:
        ...
        search_logger.debug(
            f"Using model from provider config: {model} with provider: {provider}"
        )
        return model
...
def extract_code_blocks(
    ...
    try:
        ...
        enable_prose_filtering = (
            get_credential_sync("ENABLE_PROSE_FILTERING", "true").lower() == "true"
        )
        ...
        enable_diagram_filtering = (
            get_credential_sync("ENABLE_DIAGRAM_FILTERING", "true").lower() == "true"
        )
        enable_contextual_length = (
            get_credential_sync("ENABLE_CONTEXTUAL_LENGTH", "true").lower() == "true"
        )
...
"""
    ...
    try:
        ...
        try:
            ...
            search_logger.error(
                f"Failed to create LLM client: {e} - returning default values"
            )
            return {
    ...
    for i in range(0, total_items, batch_size):
        ...
        for idx, embedding in enumerate(embeddings):
            if embedding and not all(v == 0.0 for v in embedding):
                ...
                single_embedding = await create_embedding_async(
                    batch_texts[idx], provider=provider
                )
