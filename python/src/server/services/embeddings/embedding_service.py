    ...
    try:
        ...
        try:
            ...
            search_logger.warning(
                "create_embedding called from async context - using zero embedding fallback"
            )
        ...
        search_logger.warning(
            f"Embedding creation failed, using zero fallback: {str(e)}"
        )
...
async def create_embedding_async(
    ...
    try:
        ...
        # Enhanced logging for zero embedding fallback
        search_logger.warning(
            f"Async embedding creation failed, using zero fallback: {str(e)}"
        )
        search_logger.warning(f"Failed text preview: {text[:100]}...")
...
def create_embeddings_batch(
    ...
    try:
        ...
        try:
            ...
            search_logger.warning(
                "create_embeddings_batch called from async context - using zero embedding fallback"
            )
            search_logger.warning(
                f"Batch size: {len(texts)}, first text preview: {texts[0][:100] if texts else 'empty'}..."
            )
        ...
        # Enhanced logging for zero embedding fallback
        search_logger.warning(
            f"Batch embedding creation failed, using zero fallback: {str(e)}"
        )
        search_logger.warning(
            f"Batch size: {len(texts)}, first text preview: {texts[0][:100] if texts else 'empty'}..."
        )
    ...
    for i, text in enumerate(texts):
        if not isinstance(text, str):
            ...
            try:
                ...
                search_logger.error(
                    f"Failed to convert text at index {i} to string: {e}"
                )
        ...
        try:
            async with get_llm_client(
                ...
                try:
                    ...
                    search_logger.warning(
                        f"Failed to load embedding batch size: {e}, using default"
                    )
                    batch_size = 100
                ...
                for i in range(0, len(texts), batch_size):
                    ...
                    batch_tokens = (
                        sum(len(text.split()) for text in batch) * 1.3
                    )  # Rough estimate
                        ...
                        while retry_count < max_retries:
                            try:
                                ...
                                response = await client.embeddings.create(
                                    model=embedding_model, input=batch, dimensions=1536
                                )
                                ...
                                if "insufficient_quota" in error_message:
                                    ...
                                    if retry_count < max_retries:
                                        ...
                                        search_logger.warning(
                                            f"Rate limit hit (not quota), waiting {wait_time}s before retry {retry_count}/{max_retries}"
                                        )
                                        await asyncio.sleep(wait_time)
                                    else:
                                        search_logger.error(
                                            f"Max retries exceeded for batch {i // batch_size + 1}"
                                        )
